...
def main():
    """Run all MNIST classification experiments."""
    
    # Configuration
    DATA_DIR = './mnist_data'  # Update with your MNIST data directory
    TRAIN_RATIO = 0.8
    ...
    print("="*70)
    knn_results = run_knn_experiments(X_train_flat, y_train, 
                                     X_test_flat, y_test, 
                                     k_values=[1, 3, 5])
    ...
    print("="*70)
    nb_results = run_naive_bayes_experiment(X_train_flat, y_train,
                                           X_test_flat, y_test)
    all_results['naive_bayes'] = nb_results['accuracy']
    
    # 3. Linear Classifier (NumPy)
    print("\n" + "="*70)
    print("EXPERIMENT 3: Linear Classifier (NumPy)")
    print("="*70)
    linear_numpy_results = run_linear_experiments(X_train_flat, y_train,
                                                  X_test_flat, y_test,
                                                  use_pytorch=False)
    all_results['linear_numpy'] = linear_numpy_results['accuracy']
    
    # 4. Linear Classifier (PyTorch)
    print("\n" + "="*70)
    print("EXPERIMENT 4: Linear Classifier (PyTorch)")
    print("="*70)
    linear_pytorch_results = run_linear_experiments(X_train_flat, y_train,
                                                    X_test_flat, y_test,
                                                    use_pytorch=True)
    ...
    print("="*70)
    mlp_results = run_mlp_experiment(X_train_flat, y_train,
                                    X_test_flat, y_test,
                                    hidden_dims=[256, 128],
                                    epochs=50)
    ...
    print("="*70)
    cnn_results = run_cnn_experiment(X_train_flat, y_train,
                                    X_test_flat, y_test,
                                    epochs=30)
...
def plot_results_comparison(results):
    ...
    accuracies = [
        ...
        results['knn'][5],
        results['naive_bayes'],
        results['linear_numpy'],
        results['linear_pytorch'],
        results['mlp'],
        results['cnn']
    ]
